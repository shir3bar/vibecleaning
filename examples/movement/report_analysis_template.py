...
def build_markdown_report(target_artifact, user, screenshot_mode, issue_sections, snapshots_by_key, selected_count, second_opinion_individuals=None):
    second_opinion_individuals = list(second_opinion_individuals or [])
    lines = [
        ...
        f"- Second-opinion queue: {', '.join(second_opinion_individuals) if second_opinion_individuals else 'none'}",
...
def build_html_report(target_artifact, user, screenshot_mode, issue_sections, snapshots_by_key, selected_count, second_opinion_individuals=None):
    second_opinion_individuals = list(second_opinion_individuals or [])
    parts = [
        "<!DOCTYPE html>",
        '<html lang="en">',
        ...
        f"<li><strong>Second-opinion queue:</strong> {html_escape(', '.join(second_opinion_individuals) if second_opinion_individuals else 'none')}</li>",
...
def build_individual_profile_markdown_report(target_artifact, user, sections, snapshots_by_key=None, second_opinion_individuals=None):
...
def build_individual_profile_html_report(target_artifact, user, sections, snapshots_by_key=None, second_opinion_individuals=None):
    snapshots_by_key = snapshots_by_key or {}
    second_opinion_individuals = list(second_opinion_individuals or [])
    parts = [
        "<!DOCTYPE html>",
...
def main():
        ...
        second_opinion_individuals = sorted({
            str(item).strip()
            for item in params.get("second_opinion_individuals", [])
            if str(item).strip()
        })
        ...
        if report_type == "issue_first":
            ...
            write_json(summary_path, {
                ...
                "second_opinion_individuals": second_opinion_individuals,
        ...
        if effective_output_mode == "combined":
            ...
            combined_markdown = build_individual_profile_markdown_report(
                target_artifact, user, sections, snapshots_by_key, second_opinion_individuals
            )
            ...
            for section in sections:
                ...
                if markdown_name not in output_by_name or html_name not in output_by_name:
                    raise SystemExit("Per-individual outputs were not declared")
                markdown_text = build_individual_profile_markdown_report(
                    target_artifact, user, [section], snapshots_by_key, second_opinion_individuals
                )
