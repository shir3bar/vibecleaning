...
def fallback_snapshot_data_url(example):
    ...
    circles = []
    for index, ((x, y), point) in enumerate(zip(projected, points)):
    ...
    svg = "\n".join(
        [
            ...
            '<stop offset="100%" stop-color="#dfeef6" />',
            "</linearGradient>",
            "</defs>",
            f'<rect width="{width}" height="{height}" rx="20" fill="url(#bg)" />',
            f'<rect x="{padding - 10:.2f}" y="{padding - 10:.2f}" width="{width - (padding - 10) * 2:.2f}" height="{height - (padding - 10) * 2:.2f}" rx="16" fill="#f8fcfe" stroke="#bfd5e2" />',
            f'<path d="{path_d}" fill="none" stroke="#183447" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" />',
            *circles,
            f'<text x="{padding:.2f}" y="28" font-size="18" font-family="Arial, sans-serif" fill="#102a43">{title}</text>',
            f'<text x="{padding:.2f}" y="{height - 14:.2f}" font-size="14" font-family="Arial, sans-serif" fill="#486581">{subtitle}</text>',
            "</svg>",
        ]
    )
