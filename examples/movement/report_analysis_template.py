...
def build_markdown_report(target_artifact, user, screenshot_mode, issue_sections, snapshots_by_key, selected_count, needs_check_individuals=None):
...
def build_html_report(target_artifact, user, screenshot_mode, issue_sections, snapshots_by_key, selected_count, needs_check_individuals=None):
    needs_check_individuals = list(needs_check_individuals or [])
    parts = [
        "<!DOCTYPE html>",
        '<html lang="en">',
        ...
        f"<li><strong>Needs check:</strong> {html_escape(', '.join(needs_check_individuals) if needs_check_individuals else 'none')}</li>",
...
def main():
        ...
        needs_check_individuals = sorted({
            str(item).strip()
            for item in params.get("needs_check_individuals", [])
            if str(item).strip()
        })
        ...
        if report_type == "issue_first":
            ...
            write_json(summary_path, {
                ...
                "needs_check_individuals": needs_check_individuals,
        ...
        if effective_output_mode == "combined":
            ...
            combined_markdown = build_individual_profile_markdown_report(
                target_artifact, user, sections, snapshots_by_key, needs_check_individuals
            )
            ...
            for section in sections:
                ...
                markdown_text = build_individual_profile_markdown_report(
                    target_artifact, user, [section], snapshots_by_key, needs_check_individuals
                )
