...
def main():
    ...
    for index, requested in enumerate(requested_confirmations, start=1):
        ...
        expected_count = int(requested.get("fix_count") or 0)
        if expected_count and len(fix_keys) != expected_count:
            raise SystemExit(f"Some selected fixes were not found for suspected issue: {parent_id}")
        for fix_key in fix_keys:
            context = row_context_by_number.get(fix_key_row_number(fix_key))
            if context is None:
            ...
            if (parent_id, f"row:{fix_key_row_number(fix_key)}") in already_confirmed:
